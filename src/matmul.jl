...
*(hssA::HssMatrix, B::AbstractMatrix) = mul!(similar(B, size(hssA, 1), size(B, 2)), hssA, B, 1.0, 0.0)
*(A::AbstractMatrix, hssB::HssMatrix) = copy(mul!(similar(A, size(hssB, 2), size(A, 1)), hssB', copy(A'), 1.0, 0.0)')
...
signal_mismatch_error() = throw(DimensionMismatch("clusters of hssA and hssB must be matching"))
